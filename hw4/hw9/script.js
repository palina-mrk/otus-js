...
document.querySelector('#task2').onclick = () => {
  ...
  if(!(R > 0))
    ...
    console.log(`Circle: R = ${R}, S = ${+calcCircleArea(R).toFixed(3)}, l = ${+calcCircleLength(R).toFixed(3)}`);
}
...
function solveConstantEquation (c){
  return (c ? {} : {x0: 'every number'});
}

function solveLinearEquation (b, c){
  if(!b)
    return solveConstantEquation(c);
...
function showSolutions (obj){
  ...
  const str = Object.entries(obj).map((el) => `${el[0]} = ${+el[1].toFixed(3)}`).join(', ');
